defmodule AppCrawler.Crawler do
  ...
  def separate_functions(all_links) do
      [ all_links
        # |> Enum.map(fn link -> IO.inspect(String.valid?(link)) end)
        # |> Enum.map(&to_string/1)
        # |> Enum.map(fn link -> HTTPoison.get(link) end)
      ]
  end

  defp content_website(url) do
